...
// Generated 05-Aug-2021 18:23:27
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"ma_controller\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(ma_controller_M, (NULL));
        ma_controller_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(ma_controller_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(ma_controller_M)
                == (NULL)) || rtmGetStopRequested(ma_controller_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(ma_controller_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
